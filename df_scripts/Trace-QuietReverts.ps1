<#
    ...
    AI-BLOCK OUTPUT:
    {
      "tool_name": "Trace-QuietReverts",
      "version": "3.1.0",
      "output_format": "dual",
      "human_readable": {
        "summary": "Quiet reverts trace completed",
        "window_start": "",
        "window_end": "",
        "reverted_services": [],
        "kit_refusals": [],
...
function Write-AIOutput {
    ...
    $aiOutput = @{
        ...
        human_readable = @{
            ...
            reverted_services = $Data.reverted_services
            kit_refusals = $Data.kit_refusals
            smoking_gun_events = $Data.smoking_gun_events
            medic_activity = $Data.medic_activity
...
# Store window info for AI output
$aiData.window_start = $since.ToString('yyyy-MM-dd HH:mm:ss')
$aiData.window_end = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
...
if (Test-Path $LogFile) {
    ...
    if ($refused.Count) {
        ...
        $aiData.kit_refusals = @($refused | Select-Object -Last 12)
    } else {
...
$aiData.reverted_services = $reverted
    ...
    foreach ($e in $ev) {
        ...
        foreach ($w in $Watched) {
            if ($msg -match $w -or $e.Properties[0].Value -eq $w) {
                ...
                $found7040++
                $aiData.smoking_gun_events += "{0:HH:mm:ss}  {1}" -f $e.TimeCreated, $msg
                break
            }
        }
    }
...
foreach ($ln in @('Microsoft-Windows-WaaSMedic/Operational')) {
    try {
        ...
        foreach ($e in ($ev | Select-Object -First 15)) {
            ...
            $medic++
            $aiData.medic_activity += "{0:HH:mm:ss}  [{1}] {2}" -f $e.TimeCreated, $e.Id, (($e.Message -replace '\s+',' ').Trim() -replace '^(.{150}).*','$1...')
        }
    } catch { }
}
if ($medic -eq 0) {
...
try {
    ...
    foreach ($e in $ev) {
        ...
        if ($p -match 'UpdateOrchestrator|WaaSMedic|InstallService|PushToInstall|WindowsUpdate|Search') {
            ...
            $tasks++
            $aiData.scheduled_tasks += "{0:HH:mm:ss}  {1}" -f $e.TimeCreated, $p
        }
    }
} catch { }
...
if ($reverted.Count -eq 0) {
    ...
    if ($reverted -contains 'WaaSMedicSvc') {
        ...
        $aiData.status = "explicit_changes"
    } else {
