<#
    ...
    AI-BLOCK OUTPUT:
    {
      "tool_name": "Set-NIC-USB-IRQ-Affinity",
      "version": "1.0.0",
      "output_format": "dual",
      "human_readable": {
        "summary": "NIC/USB IRQ affinity set",
        "target_cores": [],
        "devices_processed": [],
      ...
      "ai_structured": {
        ...
        "reboot_required": true,
        "status": "completed",
        "timestamp": "2026-07-26T11:51:00Z",
...
function Write-AIOutput {
    ...
    $aiOutput = @{
        ...
        human_readable = @{
            ...
            target_cores = $Data.target_cores
...
if ($TargetCores.Count -lt 1) {
    ...
    Write-AIOutput @{
        summary = "ABORTED: no usable background core on this machine ($Limit logical processors)."
        target_cores = @()
        devices_processed = @()
        reboot_required = $true
        status = "error"
...
if (-not $targets) { 
    Write-Host "No PCI NIC or USB controllers found." -ForegroundColor Yellow
    Write-AIOutput @{
        summary = "No PCI NIC or USB controllers found."
...
foreach ($t in $targets) {
    ...
    if ($Simulated) {
        ...
        $devicesProcessed += "$($t.Type): $($t.Name) - Would use CPU $core"
        $i++
        continue
    }
    try {
        ...
        $devicesProcessed += "$($t.Type): $($t.Name) - Using CPU $core"
    } catch {
        Write-Host ("  [{0}] {1} - FAILED ({2})" -f $t.Type, $t.Name, $_.Exception.Message) -ForegroundColor Yellow
        $devicesProcessed += "$($t.Type): $($t.Name) - FAILED ($($_.Exception.Message))"
    }
    $i++
}
