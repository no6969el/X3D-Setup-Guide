<#
    ...
    AI-BLOCK OUTPUT:
    {
      "tool_name": "Post-Race-Restore",
      "version": "3.1.0",
      "output_format": "dual",
      "human_readable": {
        "summary": "Post-race restore completed",
        "snapshot_used": false,
        "services_restored": [],
        "tasks_restored": [],
      ...
      "ai_structured": {
        ...
        "defender_restored": false,
        "status": "completed",
        "timestamp": "2026-07-26T11:45:22Z",
...
function Write-AIOutput {
    ...
    $aiOutput = @{
        ...
        human_readable = @{
            ...
            services_restored = $Data.services_restored
            tasks_restored = $Data.tasks_restored
            defender_restored = $Data.defender_restored
            status = $Data.status
        }
        ai_structured = @{
            tool_name = "Post-Race-Restore"
...
foreach ($s in $svcList) {
    ...
    if ($ok) { 
        Write-Log ("{0}: startup type {1}{2}" -f $name, $mapped[[int]$target], $started) 'Green' 
        $servicesRestored += $name
    }
...
foreach ($t in $taskList) {
    ...
    try {
        ...
        $tasksRestored += "{0}{1}" -f $t.Path, $t.Name
    } catch {
        $failedTasks += $t
    }
}
...
if (-not $shouldReenable) {
    ...
    try {
        ...
        if ((Get-MpComputerStatus).RealTimeProtectionEnabled) { 
            Write-Log "real-time protection: back ON" 'Green' 
            $defenderRestored = $true
        }
...
Write-AIOutput @{
    ...
    defender_restored = $defenderRestored
    status = "completed"
}