...
function Get-X3DProfile {
    ...
    if (-not $NoCache -and -not $Assume) {
        ...
        if ($saved) { 
            if ($AIOnly) {
                Write-AIOutput "Profile detected" $saved
            } elseif ($HumanReadable) {
                Write-HumanOutput "Profile detected: $($saved.Model)"
            }
            return $saved 
        }
    }
    ...
    if ($AIOnly) {
        Write-AIOutput "Profile detected" $obj
    } elseif ($HumanReadable) {
        Write-HumanOutput "Profile detected: $($obj.Model)"
    }
...
# New functions for dual-format output support
...
function Write-HumanOutput {
    ...
    # Simple human-readable format - could be enhanced later with better formatting
    if ($Title) {
        Write-Host "[$Title]" -ForegroundColor Green
    }
    if ($Data) {
        if ($Data -is [hashtable] -or $Data -is [pscustomobject]) {
            $Data | Format-List | ForEach-Object { Write-Host $_.ToString() }
        } else {
            Write-Host "$Data"
        }
    }
...
function Write-AIOutput {
    ...
    # Structured AI format - JSON output with metadata
    $output = @{
        timestamp = Get-Date -Format "o"
        tool = "X3D-Profiles.ps1"
        title = $Title
        data = $Data
    }
