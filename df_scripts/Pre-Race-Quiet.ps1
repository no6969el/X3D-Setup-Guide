<#
    ...
    AI-BLOCK OUTPUT:
    {
      "tool_name": "Pre-Race-Quiet",
      "version": "3.1.0",
      "output_format": "dual",
      "human_readable": {
        "summary": "Pre-race quiet completed",
      ...
      "ai_structured": {
        ...
        "deadman_enabled": false,
        "status": "completed",
        "timestamp": "2026-07-26T11:46:26Z",
...
function Write-AIOutput {
    ...
    $aiOutput = @{
        ...
        human_readable = @{
            ...
            services_disabled = $Data.services_disabled
            tasks_disabled = $Data.tasks_disabled
            defender_disabled = $Data.defender_disabled
            deadman_enabled = $Data.deadman_enabled
            status = $Data.status
        }
        ai_structured = @{
            tool_name = "Pre-Race-Quiet"
...
foreach ($s in $snapTasks) {
    ...
    try {
        ...
        $tasksDisabled += "{0}{1}" -f $s.Path, $s.Name
    } catch {
        $failedTasks += $s
...
foreach ($s in $snapServices) {
    ...
    if ($disabled -and $stopped)      { Write-Log ("{0}: disabled + stopped" -f $name) 'Green'; $servicesDisabled += $name }
...
if ($Deadman) {
    ...
    if (Test-Path $restore) {
        ...
        if ($LASTEXITCODE -eq 0) { Write-Log "registered: everything restores automatically at next boot unless you restore first" 'Green'; $deadmanEnabled = $true }
...
Write-AIOutput @{
    summary = "Pre-race quiet completed"
    snapshot_created = -not $ReQuiet
    services_disabled = $servicesDisabled
    tasks_disabled = $tasksDisabled
    defender_disabled = $defenderDisabled
