...
AI-BLOCK OUTPUT:
{
  "tool_name": "Test-UndervoltStability",
  "version": "2.0.0",
  "output_format": "dual",
  "human_readable": {
    "summary": "Undervolt stability test completed",
    "cpu_info": {
      "name": "",
      "cores": 0,
  ...
  "ai_structured": {
    ...
    "status": "completed",
    "timestamp": "2026-07-26T11:52:26Z",
...
if (-not $coreList -or $coreList.Count -eq 0) { 
    Write-Host "No valid cores selected." -ForegroundColor Red
    Write-AIOutput @{
        summary = "No valid cores selected"
        cpu_info = @{
            name = $cpu.Name.Trim()
            cores = $physCores
            logical_processors = $logCores
            smt_enabled = $smtOn
            base_mhz = $script:baseMHz
...
# --- AI Output ---
$aiResults = @{
    total_cores = $coreList.Count
    passed_cores = ($results.Values | Where-Object { $_.Status -eq 'pass' } | Measure-Object).Count
    failed_cores = $failed.Count
    whea_errors = ($results.Values | Measure-Object -Property Whea -Sum).Sum
    peak_mhz = ($results.Values | Measure-Object -Property PeakMHz -Maximum).Maximum
    total_run_time = Format-HMS $totalElapsed
}
